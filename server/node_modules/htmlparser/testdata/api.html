...
<head> 
	...
	<style type='text/css'> 
      ...
      line-height:1.3;
		...
		#man h1
		{
			background:url("https://nodejs.org/logo.png") no-repeat scroll center 0 transparent;
			height:111px;
			margin:15px 0 20px;
			text-align:center;
			text-indent:-2000px;
...
<p>If the connection is secure, but no explicit CA certificates are passed in the credentials, then node.js will default to the publicly trusted list of CA certificates, as given in https://mxr.mozilla.org/mozilla/source/security/nss/lib/ckfw/builtins/certdata.txt</p>
...
https://mxr.mozilla.org/mozilla/source/security/nss/lib/ckfw/builtins/certdata.txt</p>
...
<p><code>'https://user:pass@host.com:8080/p/a/t/h?query=string#hash'</code></p>

<ul>
<li><p><code>href</code></p>

<p>The full URL that was originally parsed. Example:
<code>'https://user:pass@host.com:8080/p/a/t/h?query=string#hash'</code></p></li>
...
<p><code>node-waf</code> is just https://code.google.com/p/waf/[WAF], the python-based build system. <code>node-waf</code> is
